"""Shared sole-patch animation for the ``left_shoe_*`` examples.

Animates the oriented sole contact rectangle, its origin, and its outward normal
through the whole capture. Identical across examples; the CLI builds the demo and
calls :func:`animate`.
"""
...
def animate(demo: Demonstration[ExampleTracks]) -> None:
    ...
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    ax.set_title("sole patch")

    # The frame counter lives in a small text artist updated each frame.
    frame_text = ax.text2D(0.02, 0.98, "", transform=ax.transAxes, va="top", fontsize=9)

    def update(frame: int) -> tuple[object, object, object, object]:
        ...
        frame_text.set_text(f"sole frame {frame} / {num_frames - 1}")
        return rect, origin_dot, normal_line, frame_text

    # Full redraw per frame (blit=False): blitting a 3D axis skips the 3D->2D reprojection,
    # which freezes the geometry in place. One small 3D axis redraws fast enough for
    # real-time interactive playback, so a full redraw is the correct, simple choice here.
    # Keep a reference to the animation so it is not garbage-collected before show().
    _anim = FuncAnimation(fig, update, frames=num_frames, interval=33, blit=False)
