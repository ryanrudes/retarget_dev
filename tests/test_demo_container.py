...
from retarget.demo.authoring import TypedDemonstration, TypedDemonstrationView
...
def test_load_pattern_returns_demonstration() -> None:
    ...
    mocap = _mocap_track(demo[GroundEstimationTrackId.MOCAP])
...
def test_track_lookup_by_enum_id() -> None:
    ...
    assert demo[GroundEstimationTrackId.MOCAP] is mocap
...
def test_slice_time_returns_demonstration_view() -> None:
    ...
    mocap = _mocap_track(clip[GroundEstimationTrackId.MOCAP])
...
def test_with_contacts_attaches_contact_track() -> None:
    ...
    mocap_track = _mocap_track(demo_with_contacts[GroundEstimationTrackId.MOCAP])
    assert mocap_track.contacts is contacts
    contact_track = demo_with_contacts[GroundEstimationTrackId.CONTACTS]
...
def test_enum_keyed_lookup_returns_mocap_track() -> None:
    mocap = make_mocap_track()
    demo = _make_demo(mocap)
    result = demo[GroundEstimationTrackId.MOCAP]
    assert isinstance(result, MocapTrack | MocapTrackView)
    assert result is mocap


def test_enum_keyed_view_lookup_returns_sliced_mocap_view() -> None:
    demo = _make_demo()
    clip = demo.slice_time(0.0, 0.2)
    result = clip[GroundEstimationTrackId.MOCAP]
...
def test_typed_demonstration_compiles_track_ids_and_resolves_strings() -> None:
    ...
    assert demo.tracks["mocap"] is mocap
    assert demo.tracks["contacts"] is contacts
    assert demo._get_track(demo._generated_ids.tracks.mocap) is mocap
    assert demo._get_track(GroundEstimationTrackId.MOCAP) is mocap
...
def test_typed_demonstration_rejects_unknown_string_track_names() -> None:
    ...
    with pytest.raises(KeyError, match="missing"):
        demo._get_track("missing")


def test_typed_demonstration_bridge_survives_slice_and_resample() -> None:
    demo = _typed_mocap_demo()
    clip = demo.slice_time(0.0, 0.2)
    assert isinstance(clip, TypedDemonstrationView)

    assert isinstance(clip.tracks["mocap"], MocapTrackView)
    assert clip.tracks["mocap"] is clip._get_track("mocap")
    assert clip._get_track(GroundEstimationTrackId.MOCAP) is clip._get_track("mocap")

    resampled = clip.resample_to("mocap")
    assert isinstance(resampled, TypedDemonstrationView)
    assert isinstance(resampled.tracks["mocap"], MocapTrackView)
    assert resampled.tracks["mocap"] is resampled._get_track("mocap")
    assert resampled._get_track(GroundEstimationTrackId.MOCAP) is resampled._get_track(
        "mocap"
    )
