...
`new_api_example.py` is the preferred public TypedDict authoring example. It uses the typed demo-track authoring path and the statically typed item-access bridge for the canonical mocap retrieval path, then shows the optional local-bag handoff when `bags/ground_estimation/unbagged/` exists. `demo.typed_tracks["mocap"]` is the typed path; `demo.get_track("mocap")` remains the dynamic path. `backend_specs/` contains low-level backend/manual support for real VSK-derived calibration and bag-data loading. `run_demo_track_workflow.py` demonstrates the real-data demo loader and uses backend/manual Vicon calibration support. `run_core_geometry_basics.py` is a backend/manual geometry query example.
